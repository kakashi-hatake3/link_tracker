...
from src.scrapper.models import AddLinkRequest, RemoveLinkRequest, LinkResponse, ListLinksResponse
...
class ScrapperClient:
    ...
    async def add_link(self, chat_id: int, url: HttpUrl, description: Optional[str] = None) -> Optional[LinkResponse]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(f"{self.base_url}/links", headers=headers, json=jsonable_encoder(request)) as response:
    ...
    async def remove_link(self, chat_id: int, url: HttpUrl) -> Optional[LinkResponse]:
        ...
        async with aiohttp.ClientSession() as session:
            headers = {"Tg-Chat-Id": str(chat_id)}
            async with session.delete(f"{self.base_url}/links", headers=headers, json=jsonable_encoder(request)) as response:
                if response.status == 200:
                    return LinkResponse.model_validate(await response.json())
                return None
